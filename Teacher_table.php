...
$file_sql = "SELECT * FROM teacher_files WHERE code = '$random_code'";
$file_result = $conn->query($file_sql);

if ($file_result->num_rows == 1) {
    $file_row = $file_result->fetch_assoc();
    $file_image_name = $file_row['file_path'];

   
    $file_image_path = $file_image_name;

  
    echo '<div class="container">';
    echo '<img class="table_img" src="' . $file_image_path . '" alt="صورة الطالب">';
    echo '</div>';
} else {
    echo '<div class="container">';
    echo "<h2> لم يتم وضع جدول حتي الان  </h2>";
    echo '</div>';
}
