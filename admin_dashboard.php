...
<div class="container">
    ...
    <table>
        <div>
        <a href="add_account_admin.php"> <i class="fa-solid fa-user-plus"></i> Add Person</a>
        </div>
        <thead>
            <tr>
                <th>email</th>
        ...
        <tbody>
            ...
            $sql = "SELECT family_name, password, email, phone, email,role, random_code, id FROM users";
            $result = $conn->query($sql);
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                        <td>" . $row["email"] . "</td>
                        <td>" . $row["role"] . "</td>
                        <td><a href=''>" . $row["random_code"] . "</a></td>
                        <td><a onclick='confirmDelete(" . $row["id"] . ")'>
                        <i class='fa-regular fa-trash-can' style='color:#E74C3C'></i></a>
                        <a href='random_code_admin.php?code=" . $row["random_code"] . "'><i class='fa-solid fa-pen-to-square'></i></a></td>
                        </td>
                    </tr>";
                }
            } else {
...
function confirmDelete(recordId) {
    ...
    if (result) {
        ...
        window.location.href = 'delete_admin.php?id=' + recordId;
    } else {
